...
> 文中形如 `[117](...)`、`[99](...)` 的交叉連結指向該 repo 同目錄的文件,在本 skill 內不可點。
> 內容以該 repo 為準;這裡留一份是為了讓 skill 自足。
...
這裡有個值得記住的後續:當時我們同時上了「μ=5.0 高摩擦材質」,並長期以為抓握是靠摩擦撐住的。[117](117-isaac-scene-backup-20260726.md) 的離線 USD 解析推翻了這件事——那個材質**根本沒有 authored 摩擦值**,而且補綁 marker 早就被停用。也就是說,現行基線的抓握**完全來自 SDF 碰撞近似**,摩擦那條線從頭到尾沒生效。
