...
| [`85-retro-remake-licensing`](rules/85-retro-remake-licensing.md) | 復古 remake／中文化專案的授權模型[HARD]:非商業免費（含修改再散布）、商業洽談;第一個公開 commit 前就放;授權要出現在四個位置（`LICENSE`、README、每個發行包、AppImage doc）;叫 source-available 不叫 open source;不把原版素材寫進授權範圍;附中英範本（第 2 版：實況影片明示允許、貢獻回授）;PolyForm 已取消 |
