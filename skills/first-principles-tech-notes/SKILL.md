...
6. **柵欄原則(Chesterton's Fence)**:解釋既有設計 / 老程式碼 / 奇怪規格時,先重建「當初為什麼非這樣不可」,別把 legacy 寫成「前人愚蠢」。看似不合理的設定常是某個**隱藏第一性原理**演化出的解;先說清它擋住什麼、解決什麼根本問題,再談怎麼改進。第一性原理既用來從零推導,也用來逆向重建既有設計的必然性。
