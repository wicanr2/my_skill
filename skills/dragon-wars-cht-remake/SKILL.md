...
description: 推進《火龍之戰》(Dragon Wars, 1989)繁中化 + C++20/SDL2 重寫(opendw_remake):用 opendw 反組譯當 oracle 做逐指令差異測試、DATA1/DATA2 資產萃取(5-bit 文字/Huffman/res31/sprite/場景圖去交錯)、Read Paragraph 防拷、CJK 24×24 渲染。觸發:「火龍之戰」「Dragon Wars」「opendw」「opendw_remake」「火龍之戰中文化」,或接續萃取/翻譯/重寫/驗證。
---
...
## 專案座標
- **Repo**:`github.com/wicanr2/opendw_dragon_wars_cht`(本機 `~/tmp/longcat/opendw_dragon_wars_cht/`)
- **oracle 原始碼**:`~/tmp/longcat/opendw/`(Devin Smith C 反組譯,**唯讀,當對照**)
