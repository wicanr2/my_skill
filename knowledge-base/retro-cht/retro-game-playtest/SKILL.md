...
## [HARD] 測試「自毀型流程」的陷阱(self-destruct on wrong input)

有些畫面**按錯鍵就把遊戲弄死**(年齡/密碼驗證答錯 → 踢出遊戲/回主選單;dialog 選錯 → 存檔覆寫/退出)。
對這種流程用 **headless 盲注按鍵(xdotool 定時送 Return/字母)測試極脆弱**:時序差一格就選到錯項,遊戲自毀,
每次重來燒掉大量循環(LSL1 年齡驗證繁中化撞了十幾輪:Return 一按到年齡選單就選 under-15 被踢黑屏)。

- **測前先問**:被測畫面會不會 self-destruct on wrong input?會 → 別用盲注定時按鍵。
- **兩條出路**:(a) 找**非破壞性入口**(存檔直接進目標房、debug 換場、跳過該畫面的旗標);(b) 更好——
  **讓功能本身繞過破壞路徑**(要略過驗證 → 直接跳門後房間,而非模擬答對每一題,見 `re-retro-cht-rulebook`/SKILL 增量六)。
- 若必須經過:合成鍵注入要**優先於玩家真實鍵**、且在畫面真正就緒(非片頭/過場)時才送;每步截圖確認狀態再送下一鍵。
- 對應 `rulebook/41`(特例/循環暴增 = 停下重想:自毀畫面難測本身常是「該換繞過方案」的信號)。
